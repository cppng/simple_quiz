...
const scoreCardLoser = document.querySelector("#score-card-loser");
const scoreCardWinner = document.querySelector("#score-card-loser"); 
const scoreCardExcellent = document.querySelector("#score-card-excellent");
...
function hideCards() {
  ...
  scoreCardExcellent.setAttribute("hidden", true);
  scoreCardLoser.setAttribute("hidden", true);

  scoreCardWinner.setAttribute("hidden", true);  
}
...
function endQuiz() {
  console.log("user scored " + UserScore);
  clearInterval(intervalID);
  hideCards();
  
  mscoreCard = null;
  if(UserScore < questions.length/3){
    mscoreCard = scoreCardLoser;
  }
  else if(UserScore > questions.length - 3){
    mscoreCard = scoreCardExcellent;
  }
  else{
    mscoreCard = scoreCardWinner;
  }

  mscoreCard.removeAttribute("hidden");

  let summary = ""+ UserScore + "/"+questions_length;
  let displayDom = document.getElementsByClassName("score");
  console.log(displayDom);
  for(let i = 0; i < displayDom.length; i++){
      element = displayDom[i];
